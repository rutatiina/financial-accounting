...
txn_type = function () {
    ...
    var config = function(state) {
        ...
        var table_rows = $('#txn_type_field_rows tr');

        var rows = table_rows.length; //console.log(rows);
        var index = rows - 1;

        if (rows > 1 && state === false) { //alert('there');
            table_rows.not(".txn_type_row_template").find("select").select2({});
            return true;
        }

        var rg_clone = $(".txn_type_row_template").clone();

        //console.log(rg_clone);
        //console.log(select2_items);

        //remove the item_row_template class
        rg_clone.removeClass('txn_type_row_template hidden');
        ...
        rg_clone.appendTo("#txn_type_field_rows");

        rg_clone.find("select").select2({});
    }

    var txn_type_config = function(config) {
    ...
    var datatable_txn_type = function() {
        ...
        var dtable = table.DataTable({
            ...
            aoColumns: [
                ...
                { "mDataProp": "category", "sClass": "details" },
                { "mDataProp": "show_payment_instructions", "sClass": "details" },
                { "mDataProp": "show_terms_and_conditions", "sClass": "details" },
                { "mDataProp": null, "sClass": "text-center" }
            ],
            fnRowCallback: function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                var action = '<ul class="icons-list">\
                    <li><a href="'+APP_URL+'/financial-accounts/settings/txn-type/' + aData['id'] + '/edit"><i class="icon-pencil7"></i></a></li> \
                </ul>';
                ...
                $(nRow).find('td.details').click(function() {
                    ...
                    row.child(txn_type_config(aData['config'])).show();
                    tr.addClass('shown');
