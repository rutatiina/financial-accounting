...
rg_payment = function () {
    ...
    var datatable_txns = function () {
        ...
        var dtable = _table.DataTable({
            ...
            fnRowCallback: function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                ...
                $('td:eq(3)', nRow).html(aData.credit_account.name);
                $('td:eq(6)', nRow).html(rg_number_format(aData['total']) + ' ' + aData['base_currency']);
        ...
        $('.navbar').on('click', '.rg_datatable_selected_delete, .rg_datatable_row_delete', function (ev) {
            ...
            rutatiina.transaction_delete({
                ...
                title: "Are you sure?",
                text: "You will not be able to recover the transaction(s)!",
                type: "warning",
    ...
    var contact_txns = function () {
        ...
        $('body').on('change', '[name="contact_id"]', function () {
            ...
            $.ajax({
                url: selectField.data('bills-url'),
                method: 'POST',
        ...
        $('#base_currency').change(function () {
            ...
            if (txns.length == 0) {
                ...
                $.each(txns, function (index, value) {
                    if (value['base_currency'] == selected.id) {
                        ...
                        <tr>\
                            ...
                            <td class="">\
                                <div class="text-semibold">' + value['number'] + '</div>\
                        ...
                        /*
                                    <div class="input-group col-lg-12">\
                                        <select name="items['+value['id']+'][txn_entree_id]" class="bootstrap-select input-roundless" data-width="150px">\
                                            ...
                                            <option value="70">Forex Gain</option>\
                                            <option value="72">Forex Loss</option>\
                                        </select>\
    ...
    var allocated_amount = function (amount) {
        var html = 'Total Amount paid: ' + rg_number_format(amount);
    ...
    var bill_payment_info = function () {
    ...
    var pay_all_fully = function () {
        $('.pay_all_fully').click(function (e) {
            e.preventDefault();
            e.stopPropagation();
            var tatal_max = 0;
            $.each($('#contact_txns tr'), function (index, tr) {
    ...
    var total_details = function () {
        ...
        if (String(total_received) == String(total_rate)) {
