        ...
        <div class="content no-border no-padding">
            ...
            <div class="panel panel-flat no-border no-shadow no-padding">
                ...
                <div class="panel-body no-padding">
                    ...
                    <form id="txn_form" action="{{route('accounting.purchases.debit-notes.update', $txn->id)}}" method="post" data-edit="true" class="form-horizontal" style="margin-bottom: 100px;">
                        @csrf
                        @method('PATCH')
                                            ...
                                            @foreach($contacts as $category => $_contacts)
                                                    <optgroup label="{{$category}}">
                                                        @foreach($_contacts as $contact)
                                                            <option value="{{$contact->id}}" {{($contact->id == $txn->debit_contact_id ) ? 'selected' : ''}} data-currency="{{$contact->currency}}" data-exchange_rates="{{json_encode(RgForex::contactExchangeRate($contact))}}" data-currencies="{{ implode(',', $contact->currencies)}}">{{$contact->display_name}}</option>
                                                        @endforeach
                                                    </optgroup>
                                                @endforeach
                                        </select>
