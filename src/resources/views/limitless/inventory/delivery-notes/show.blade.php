...
@section('head')
    <script src="{{ mix('/template/limitless/layout_2/LTR/default/assets/mix/delivery_note.js') }}"></script>
@endsection
        ...
        <div class="sidebar-content" style="width: 389px;">
                ...
                <div class="page-header-content">
                    <div class="page-title clearfix">
                        ...
                        <div class="btn-group pull-left">
                            <a href="#" class="btn btn-link dropdown-toggle text-semibold">All Delivery Notes</a>
                        </div>

                        <button data-href="{{route('accounting.inventory.delivery-notes.create')}}" type="button" class="btn btn-danger btn-sm pull-right"><i class="icon-plus22"></i> New</button>
            ...
            <div class="panel panel-flat no-border ">
                ...
                <div class="table-responsive no-border datatable-invoices">
                    <table class="rg_datatable_sidebar table table-hover text-nowrap" data-ajax="{{route('accounting.inventory.delivery-notes.datatables')}}" data-txn-id="{{$txn->id}}">
                        <thead class="hidden">
                        <tr>
                ...
                <div class="page-title clearfix text-right">
                    ...
                    <div class="btn-group pull-left mr-20">
                        ...
                        <ul class="dropdown-menu dropdown-menu-left">
                            ...
                            <li class="divider"></li>
                            <li><a href="{{route('accounting.inventory.delivery-notes.destroy', $txn->id)}}" class="text-danger rg-ajax-accounting-sales-estimate-destroy" data-callback="{{route('accounting.inventory.delivery-notes.index')}}"><i class="icon-trash"></i> Delete transaction</a></li>
                        </ul>
                    </div>
                    ...
                    <div class="btn-group pull-right">
                        ...
                        <button data-href="{{route('accounting.inventory.delivery-notes.edit', $txn->id)}}" type="button" class="btn btn-default btn-icon"><i class="icon-pencil3"></i></button>
                        ...
                        <a href="{{route('accounting.inventory.delivery-notes.index')}}" class="btn btn-icon btn-default"><i class="icon-cross"></i> Back</a>
                    </div>
            ...
            <div class="panel panel-flat no-border no-shadow">
                ...
                @if (empty($txn))
                    <div class="panel-body no-padding">
                        <div class="col-md-8">
                            <button data-href="/inventory/delivery_note/" type="button" class="btn btn-danger"><i class="icon-plus22"></i> New Delivery Note</button>
                            <hr/>
					...
					<div class="panel panel-white">
						<div class="panel-heading hidden-print">
							...
							<div class="heading-elements">
								<a href="{{route('accounting.inventory.delivery-notes.copy', $txn->id)}}" class="btn btn-default btn-xs heading-btn"><i class="icon-file-check position-left"></i> Copy</a>
						    ...
						    <table class="table table-lg">
						        <thead>
						            <tr>
                                        ...
                                        <th class="col-sm-1 text-right">Batch</th>
                                    ...
                                    @foreach ($txn->items as $item)
                                        @if (!in_array($item->type, ['txn', 'txn_type', 'tax']))
                                            <tr>
                                                ...
                                                <td class="text-right">{{$item->batch}}</td>
                                                <td class="text-right">{{$item->expiry}}</td>
                                                <td class="text-right"><span class="text-semibold">{{number_format($item->quantity)}}</span></td>
                                            </tr>
                                        @endif
                                    @endforeach
						        </tbody>
