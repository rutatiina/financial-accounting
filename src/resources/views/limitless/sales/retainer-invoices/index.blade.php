...
@section('title', 'Retainer Invoices')
...
@section('content')
        ...
        <div class="page-header-content">
            <div class="page-title clearfix">
                <h1 class="pull-left no-margin text-light">
                    <i class="icon-file-plus position-left"></i> Retainer Invoices
                    <small></small>
                </h1>
                <div class="pull-right">
                    <a href="{{route('accounting.sales.retainer-invoices.create')}}" type="button" class="btn btn-danger pr-20"><i class="icon-plus22"></i> New Retainer Invoice </a>
                </div>
            </div>
        </div>
    ...
    <div class="content no-padding">
        ...
        <div class="panel panel-flat no-border no-shadow">
            ...
            <table class="rg-datatable-txns-table table datatable-pagination" data-ajax="{{route('accounting.sales.retainer-invoices.datatables')}}">
                <thead>
                <tr>
                    <th width="12"></th>
                    <th>DATE</th>
                ...
                @foreach($txns as $txn)
                    <tr>
                        ...
                        <td>{{$txn->date}}</td>
                        <td>
                            <a href="{{route('accounting.sales.retainer-invoices.edit', $txn->id)}}">
                                {{$txn->number}}
                            </a>
                        </td>
                        <td>{{$txn->reference}}</td>
