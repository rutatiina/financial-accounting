            ...
            <div class="page-header-content">
                <div class="page-title">
                    <h1 class="no-margin text-light">
                        ...
                        <small>Edit Credit note to a buyer.</small>
                    </h1>
                </div>
            </div>
        ...
        <div class="content no-border no-padding">
            ...
            <div class="panel panel-flat no-border no-shadow no-padding">
                ...
                <div class="panel-body no-padding">
                    ...
                    <form id="txn_form" action="{{route('accounting.sales.credit-notes.update', $txn->id)}}" method="post" class="form-horizontal" style="margin-bottom: 100px;" data-edit="true">
                                            ...
                                            <tr id="row_{{$key}}">
                                                ...
                                                <td class="no-padding">
                                                    <select class="item_row_tax rg_tax_selector" multiple data-row="{{$key}}" data-minimum-Results-For-Search="-1" data-placeholder="Select" data-allow-clear="true">
                                                        @foreach($taxes as $tax)
                                                            <option value="{{$tax->value}}" {{(in_array($tax->id, $item->taxes)) ? 'selected' : ''}} data-id="{{$tax->id}}" data-method="{{$tax->method}}" title="{{ucfirst($tax->method)}}">{{$tax->display_name}} - {{ucfirst($tax->method)}}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td class="no-padding">
                ...
                <div class="navbar-collapse collapse" id="navbar-second">
                    ...
                    <div class="navbar-text">
                        ...
                        <div class="btn-group dropup btn-xs btn-group-animated no-padding mr-20">
                            <button type="button" class="btn btn-danger btn-xs pr-20 pl-20 submit_txn_form" data-onsuccess="{{route('accounting.sales.credit-notes.show', $txn->id)}}"><i class="icon-cog5 position-left"></i> Update {{$entree->name}}</button>
                            {{--
