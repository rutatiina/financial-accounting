        ...
        <div class="sidebar-content" style="width: 389px;">
                    ...
                    <div class="page-title clearfix">
                        ...
                        <div id="quick-sort" class="btn-group pull-left">
                            <a href="#" class="btn btn-link dropdown-toggle text-semibold" data-toggle="dropdown" aria-expanded="true">
                                <span class="dropdown-title">All Credit Notes</span>
                                <span class="caret"></span>
                            </a>
                            <ul class="dropdown-menu dropdown-menu-left dropdown-menu-xs no-border-radius">
                                <li><a href="#" data-ajax="/datatable/transactions/entree/credit_note/?status=client+viewed">Client viewed</a></li>
                                <li class="divider"></li>
                                <li><a href="#" data-ajax="/datatable/transactions/entree/credit_note/">All Credit Notes</a></li>
                            </ul>
                        </div>

                        <button data-href="/sales/credit_note/" type="button" class="btn btn-danger btn-sm pull-right"><i class="icon-plus22"></i> New</button>
            ...
            <div class="panel panel-flat no-border ">
                ...
                <div class="table-responsive no-border datatable-invoices">
                    <table class="rg_datatable_sidebar table table-hover text-nowrap" data-ajax="{{route('accounting.sales.credit-notes.datatables')}}" data-txn-id="{{$txn->id}}">
                        <thead class="hidden">
                        <tr>
                            <th>CREDIT NOTES</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <h6 class="no-margin text-bold text-right">
                                    <small>UGX</small> 17,890
                                    <small class="display-block text-muted text-size-small">DRAFT</small>
                                </h6>
                            </td>
                        </tr>
                        --}}
            ...
            <div class="page-title clearfix text-right">
                ...
                <div class="btn-group pull-left mr-20">
                    ...
                    <ul class="dropdown-menu dropdown-menu-left">
                        ...
                        <li class="divider"></li>
                        <li><a href="{{route('accounting.sales.credit-notes.destroy', $txn->id)}}" class="text-danger rg_ajax_accounting_sales_credit_note_destroy" data-callback="{{route('accounting.sales.credit-notes.index')}}"><i class="icon-trash"></i> Delete transaction</a></li>
                    </ul>
                </div>
                ...
                <div class="btn-group pull-right">
                    {{--<button data-href="/sales/credit_note/" type="button" class="btn btn-danger"><i class="icon-plus22"></i> New Estimate</button>--}}
                    <button data-href="{{route('accounting.sales.credit-notes.edit', $txn->id)}}" type="button" class="btn btn-default btn-icon"><i class="icon-pencil3"></i></button>
                    ...
                    <a href="{{route('accounting.sales.credit-notes.index')}}" class="btn btn-icon btn-default"><i class="icon-cross"></i> Back</a>
                </div>
        ...
        <div class="panel panel-flat no-border no-shadow">
            ...
            @if (empty($txn))
                <div class="panel-body no-padding">
                    <div class="col-md-8">
                        <button data-href="/sales/credit_note/" type="button" class="btn btn-danger"><i class="icon-plus22"></i> New Credit Notes</button>
                        <hr/>
            ...
            <div class="panel-body">
                ...
                <div class="max-width-820" style="margin: 0px auto;">
					...
					<div class="panel panel-white">
						<div class="panel-heading hidden-print">
							...
							<div class="heading-elements">
								<a href="{{route('accounting.sales.credit-notes.copy', $txn->id)}}" class="btn btn-default btn-xs heading-btn"><i class="icon-file-check position-left"></i> Copy</a>
                    ...
                    <!-- Invoice template -->
                    <div class="panel panel-white no-border-radius hidden">
                        ...
                        <div class="panel-body no-padding-bottom">
                            <div class="row">
                                <div class="col-xs-6 content-group">
                                    <div class="row">
                                        @if (!empty($tenant->logo) && file_exists(public_path('storage/'.$tenant->logo)))
                                            <img src="{{$tenant->logo}}" class="col-xs-6 mt-15" alt="" style="max-width: 120px; max-height: 100px;">
                                        @else
                                            <img class="col-xs-6 mt-15" src="/timthumb.php?src={{asset('template/azia/img/500x500.png')}}&w=120&h=100&q=100">
                                        @endif

                                        <ul class="col-xs-6 list-condensed list-unstyled {{!empty($tenant->logo) ? 'col-xs-6' : ''}}">
                                            <li><h5>{{$tenant->name}}</h5></li>
                                ...
                                <div class="col-xs-6 content-group">
                                    <div class="invoice-details text-right">
                                        <h1 class="text-uppercase text-semibold no-margin-top">{{$txn->type->name}}</h1>
                                        <h5 class="text-danger no-margin-top">{{$txn->number}}</h5>
                                        <ul class="list-condensed list-unstyled">
                                            <li>Date: <span class="text-semibold">{{$txn->date}}</span></li>
                                        </ul>
                                    </div>
                                        ...
                                        @foreach ($txn->items as $item)
                                            @if (!in_array($item->type, ['txn', 'txn_type', 'tax']))
                                                <tr>
                                                    ...
                                                    <td class="text-right">{{number_format($item->rate, $tenant->decimal_places)}}</td>
                                                    <td class="text-right">{{number_format($item->quantity, $tenant->decimal_places)}}</td>
                                                    <td class="text-right"><span class="text-semibold">{{number_format($item->total, $tenant->decimal_places)}}</span></td>
                                                </tr>
                                            @endif
                                        @endforeach
                                    ...
                                    <div class="content-group">
                                        ...
                                        <div class="table-responsive no-border">
                                            <table class="table">
                                                <tbody>
                                                <tr>
                                                    <th class="no-border-top">SUB TOTAL</th>
                                                    <td class="no-border-top text-right">{{number_format($txn->taxable_amount, $tenant->decimal_places)}}</td>
                                                </tr>
                                                @foreach ($txn->items as $item)
                                                    @if (in_array($item->type, ['txn_type', 'tax']))
                                                        <tr>
                                                ...
                                                <tr>
                                                    <th>TOTAL ({{$txn->base_currency}})</th>
                                                    <td class="text-right"><h5 class="text-semibold">{{number_format($txn->total, $tenant->decimal_places)}}</h5></td>
                                                </tr>
                            ...
                            <h6>Amount in words:</h6>
                            <p class="text-muted amount_in_words mb-20">{{ucfirst($txn->total_in_words)}}</p>
