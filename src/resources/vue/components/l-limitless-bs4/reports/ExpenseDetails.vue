            ...
            <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
                <div class="d-flex">
                    <div class="breadcrumb">
                        ...
                        <span class="breadcrumb-item">Purchases and Expenses</span>
                        <span class="breadcrumb-item active">Expense Details</span>
                    </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-right">AMOUNT WITH TAX</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            <tr v-for="row in tableData.payload.data">
                                ...
                                <td>{{row.debit_account.name}}</td>
                                <td> </td>
                                <td class="cursor-pointer font-weight-bold text-right">
                                    <span class="text-slate-800">{{rgNumberFormat(row.taxable_amount, 2)}}</span>
                                    <small>{{row.base_currency}}</small>
                                </td>
                                <td class="cursor-pointer font-weight-bold text-right">
                                    <span class="text-danger-800">{{rgNumberFormat(row.total, 2)}}</span>
                                    <small>{{row.base_currency}}</small>
                                </td>
                            </tr>
                        </tbody>
    ...
    export default {
        ...
        watch: {
            '$route.query.page': function (page) {
                this.tableData.url = '/accounting/reports/expense-details' + '?page=' + page;
            }
        },
        mounted() {
            ...
            if (this.$route.query.page === undefined) {
                this.tableData.url = '/accounting/reports/expense-details';
            } else {
                this.tableData.url = '/accounting/reports/expense-details' + '?page='+this.$route.query.page;
            }
