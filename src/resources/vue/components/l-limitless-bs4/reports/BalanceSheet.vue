            ...
            <div class="card max-width-1280 shadow-none border-0 m-auto">
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            <tr>
                                <td colspan="3" class="font-weight-bold text-capitalize">Assets</td>
                            </tr>
                            <tr v-for="account in report.assets">
                                <td class="pl-4 text-primary">{{account.name}}</td>
                                <td class="text-right">{{rgNumberFormat(account.balance, 2)}}</td>
                            </tr>
                            <tr>
                                <td class="pl-4 font-weight-semibold">Total Assets</td>
                                <td class="text-right font-weight-semibold">{{rgNumberFormat(report.total_assets, 2)}}</td>
                            </tr>
                        </tbody>
                        <tbody>
                            <tr>
                                <td colspan="3" class="font-weight-bold text-capitalize">Liability & Equity</td>
                            </tr>
                            <tr v-for="account in report.liability_and_equity">
                                <td class="pl-4 text-primary">{{account.name}}</td>
                                <td class="text-right">{{rgNumberFormat(account.balance, 2)}}</td>
                            </tr>
                            <tr>
                                <td class="pl-5">Balancing Figure</td>
                                <td class="text-right">{{rgNumberFormat(report.balancing_figure, 2)}}</td>
                            </tr>
                            <tr>
                                <td class="pl-4">Retained Earnings</td>
                                <td class="text-right">{{rgNumberFormat(report.retained_earnings, 2)}}</td>
                            </tr>
                            <tr>
                                <td class="pl-4 font-weight-semibold">Total Liability & Equity</td>
                                <td class="text-right font-weight-semibold">{{rgNumberFormat(report.total_liability_and_equity, 2)}}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            async generate() {
                try {
                    return await axios.post('/financial-accounts/reports/balance-sheet')
                        .then((response) => {
